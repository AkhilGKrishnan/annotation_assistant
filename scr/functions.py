...
class subprocesses(common_functions):
  ...
  def run(self, cmd, cwd=''):
    ...
    self.output+=self.trace_console(p)+'\n'
...
#---/ htmlPy dashboard scripts /-----------------------------------------------
#
class Dashboard(htmlPy.Object, common_functions):

  def __init__(self, app):
    super().__init__()
    self.app = app
    self.plug_backend_and_login()
    return None

  def plug_backend_and_login(self):
    login_template_dict = self.get_login_data()
    if hasattr(self, "login_data"):
      self.plug_annotation_backend()
      if self.annotation_status_check()=='annotate':
        self.annotation_view()
      elif self.annotation_status_check()=='add new':
        self.new_text_view()
    else:
      self.templates_manager("login", login_template_dict)
  ...
  def get_login_data(self):
    login_template_dict = {"username": "", "password": "", "remember": ""}
    if not hasattr(self, "login_data"):
      if os.path.exists('data/login_data.json'):
        self.login_data = self.load_json('data/login_data.json')
        login_template_dict["remember"] = "checked"
    if hasattr(self, "login_data"):
      login_template_dict["username"] = self.login_data["username"]
      login_template_dict["password"] = self.login_data["password"]
    return login_template_dict
  ...
  def github_login(self, json_data):
    ...
    if login_check in [None, False]:
      ...
      if login_data['remember']=='on':
        self.dump(json.dumps(login_data), 'data/login_data.json')
      else:
        if os.path.exists('data/login_data.json'):
          os.remove('data/login_data.json')
      self.plug_backend_and_login()
  ...
  @htmlPy.Slot()
  def select_new_text(self):
    print('getting new text, please wait...')
  ...
  def templates_manager(self, template, args_dict):
    '''
    Shortcut to load htmlPy template.
    '''
    self.app.template = ("%s.html" %(template), args_dict)
